...
- TimelineAgent no longer uses hardcoded “kitchen/bathroom/default remodel” task templates; tasks are generated by the LLM from user-defined scope + pipeline JSON.
- TimelineAgent no longer defaults start date to “2 weeks from now”; it requires `projectBrief.timeline.desiredStart` from the Clarification JSON (otherwise returns N/A with an explicit error).
...
- **Epic 2 (Deep Agent Pipeline)**: merged (PR #17) — 19 agents, 205 tests
...
Epic 6 wires the merged epics into a single end-to-end workflow (Projects → Annotate → Estimate):

- **6-1 Project persistence + Scope restructure**: ✅ done
- **6-2 Estimate page (two-phase) + tabs + dual PDF**: 🔄 review
