...
exports.clearGlobalMaterials = exports.listGlobalMaterials = exports.seedGlobalMaterials = void 0;
const admin = require("firebase-admin");
const XLSX = require("xlsx");
const path = require("path");
const globalMaterials_1 = require("./types/globalMaterials");
const globalMaterials_2 = require("./globalMaterials");
 ...
 */
function parseExcelFile(xlsxPath) {
 ...
 */
function rowToMaterial(row, now) {
    const normalizedName = (0, globalMaterials_2.normalizeProductName)(row.item_name);
    const id = (0, globalMaterials_2.generateMaterialId)(row.item_name, globalMaterials_1.DEFAULT_ZIPCODE);
    ...
    return {
        ...
        zipCode: globalMaterials_1.DEFAULT_ZIPCODE,
        retailers: {
            lowes: row['lowes link'] ? {
                productUrl: row['lowes link'],
                productId: (0, globalMaterials_2.extractProductId)(row['lowes link'], 'lowes'),
            ...
            homeDepot: row['home depot link'] ? {
                ...
                productId: (0, globalMaterials_2.extractProductId)(row['home depot link'], 'homeDepot'),
 ...
 */
async function seedGlobalMaterials(xlsxPath) {
    ...
    console.log(`[SEED] Default zipCode: ${globalMaterials_1.DEFAULT_ZIPCODE}`);
}
exports.seedGlobalMaterials = seedGlobalMaterials;
/**
 * List all materials in the database (for verification)
 */
async function listGlobalMaterials() {
    ...
    snapshot.docs.forEach((doc, index) => {
        var _a, _b;
        const data = doc.data();
        const lowePrice = ((_a = data.retailers.lowes) === null || _a === void 0 ? void 0 : _a.price) || 'N/A';
        const hdPrice = ((_b = data.retailers.homeDepot) === null || _b === void 0 ? void 0 : _b.price) || 'N/A';
        console.log(`  ${index + 1}. ${data.name} | Lowes: $${lowePrice} | HD: $${hdPrice} | Aliases: ${data.aliases.length}`);
    });
}
exports.listGlobalMaterials = listGlobalMaterials;
/**
 * Clear all materials (for testing)
 */
async function clearGlobalMaterials() {
    const snapshot = await db.collection('globalMaterials').get();
    if (snapshot.empty) {
...
exports.clearGlobalMaterials = clearGlobalMaterials;
// CLI entry point
...
//# sourceMappingURL=seedGlobalMaterials.js.map