 ...
 */
function buildInferencePrompt(quantities, projectType, finishLevel, scopeText, clarificationData) {
 ...
 */
async function runEnhancedInference(openai, quantities, projectType, finishLevel, scopeText, clarificationData, planImageUrl) {
    var _a, _b;
    const prompt = buildInferencePrompt(quantities, projectType, finishLevel, scopeText, clarificationData);
    try {
        let response;
        if (planImageUrl) {
...
exports.runEnhancedInference = runEnhancedInference;
/**
 * Validate and fix inference result to ensure schema compliance
 */
function validateAndFixInferenceResult(raw) {
    const result = {
        roomTypes: [],
        spatialNarrative: '',
        inferredItems: [],
    ...
    if (Array.isArray(raw.roomTypes)) {
        result.roomTypes = raw.roomTypes.map((rt) => ({
            roomId: String(rt.roomId || ''),
            inferredType: validateRoomType(rt.inferredType),
    ...
    if (Array.isArray(raw.inferredItems)) {
        result.inferredItems = raw.inferredItems.map((item) => ({
    ...
    if (Array.isArray(raw.standardAllowances)) {
        result.standardAllowances = raw.standardAllowances.map((item) => ({
    ...
    if (raw.materialsAndFinishes && typeof raw.materialsAndFinishes === 'object') {
        const mf = raw.materialsAndFinishes;
        if (Array.isArray(mf.recommended)) {
            result.materialsAndFinishes.recommended = mf.recommended.map((r) => ({
        ...
        if (Array.isArray(mf.alternatives)) {
            result.materialsAndFinishes.alternatives = mf.alternatives.map((a) => ({
    ...
    if (Array.isArray(raw.scopeAmbiguities)) {
        result.scopeAmbiguities = raw.scopeAmbiguities.map((s) => ({
...
function validateRoomType(type) {
    ...
    return validTypes.includes(lower) ? lower : 'other';
}
/**
 * Get default inference result when LLM fails
 */
function getDefaultInferenceResult() {
    return {
        roomTypes: [],
 ...
 */
function mergeInferenceIntoCSI(baseItems, inference) {
    const merged = JSON.parse(JSON.stringify(baseItems));
    // Add inferred items
...
exports.mergeInferenceIntoCSI = mergeInferenceIntoCSI;
//# sourceMappingURL=enhancedInference.js.map